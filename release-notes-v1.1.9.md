...
**5. Google Drive downloads could destroy a local backup**
...
**6. ZIP write failures were ignored**
...
**7. `finalize_restore` leaked the admin finalize token**
...
**8. PHP 8.4+ deprecation in the Google Drive client**
...
**9. External CDN dependency removed**
...
**10. Google Drive calls with no timeout.** The duplicate-file lookup, the
...
**11. Upload could spin forever.** A session repeatedly answering `308` without
advancing produced an infinite loop. Now bounded at 5 stalled rounds.

**12. Upload chunk size could exhaust memory.** The 5 MB chunk is held in memory
...
**13. Drive listing errors looked like "no backups".** A non-2xx response
returned an empty file list with no error, so an expired token presented as an
empty Drive tab. The error is now surfaced.

**14. The plugin polluted its own backups.** Export-state transients were dumped
...
| Full export → archive inspection | manifest valid, no self-inclusion, no generated columns |
